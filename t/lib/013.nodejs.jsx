...
class _Test extends TestCase {
  ...
  function testFS() : void {
    var file = node.__filename;
    var dir  = node.__dirname;

    this.expect(fs.statSync(file).isDirectory()).toBe(false);
    this.expect(fs.statSync(dir).isDirectory()).toBe(true);
  }

  function testUrl() : void {
    var u = url.parse("http://example.com/foo/bar?k=x&k=y#here");
    this.expect(u.protocol).toBe("http:");
    this.expect(u.host).toBe("example.com");
    this.expect(u.hash).toBe("#here");
    this.expect(u.query).toBe("k=x&k=y");
    this.expect(u.pathname).toBe("/foo/bar");

    this.expect(url.resolve("/foo/bar", "baz")).toBe("/foo/baz");
  }

  function testPath() : void {
    var p = "/foo/bar/baz.txt";
    this.expect(path.normalize(p)).toBe("/foo/bar/baz.txt");
    this.expect(path.join('/foo', 'bar', 'baz/asdf', 'auux', '..')).toBe('/foo/bar/baz/asdf');
    this.expect(path.resolve('/foo/bar', './baz')).toBe('/foo/bar/baz');
    this.expect(path.resolve('/foo/bar', '/tmp/file/')).toBe('/tmp/file');
    this.expect(path.resolve('wwwroot', 'static_files/png/', '../gif/image.gif')).toBe(process.cwd() + '/wwwroot/static_files/gif/image.gif');
    this.expect(path.relative('/data/orandea/test/aaa', '/data/orandea/impl/bbb')).toBe('../../impl/bbb');
    this.expect(path.dirname(p)).toBe("/foo/bar");
    this.expect(path.basename(p)).toBe("baz.txt");
    this.expect(path.basename(p, '.txt')).toBe("baz");
    this.expect(path.extname('index.html')).toBe('.html');
    this.expect(path.extname('index.')).toBe('.');
    this.expect(path.extname('index')).toBe('');
  ...
  function testCreateHttpServer() : void {
    ...
    //httpd.listen(2013); // blocks

    this.expect(httpd).notToBe(null);
  }
  ...
  function testHttpRequest() : void {
    this.async((async) -> {
      ...
      http.get("http://no.such.host/foo/bar", (res) -> {
        this.pass("request finished");
        async.done();
      }).on("error", (arg) -> {
        this.pass("request finished with " + arg as string);
        async.done();
      });
    }, 4000);
  }
