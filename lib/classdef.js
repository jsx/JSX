...
var ClassDefinition = exports.ClassDefinition = Class.extend({
	...
	getMemberTypeByName: function (errors, classDefs, name, isStatic) {
		// returns an array to support function overloading
		var types = [];
		this._getMemberTypesByName(errors, classDefs, types, name, isStatic);
		switch (types.length) {
		case 0:
			return null;
		case 1:
	...
	_getMemberTypesByName: function (errors, classDefs, types, name, isStatic) {
		if (! isStatic) {
			if (this._extendClassDef != null)
				this._extendClassDef._getMemberTypesByName(errors, classDefs, types, name, isStatic);
			for (var i = 0; i < this._implementClassDefs.length; ++i)
				this._implementClassDefs[i]._getMemberTypesByName(errors, classDefs, types, name, isStatic);
		}
		for (var i = 0; i < this._members.length; ++i) {
			var member = this._members[i];
			// check that name and the static flag is same, ignore "override"
			if (name == member.name()
				&& (member.flags() & (ClassDefinition.IS_STATIC | ClassDefinition.IS_OVERRIDE)) == (isStatic ? ClassDefinition.IS_STATIC : 0)) {
