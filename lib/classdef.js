		...
		for (var i = 0; i < this._members.length; ++i) {
			if (this._members[i].name() == member.name()) {
				...
				if ((member.flags() & ClassDefinition.IS_OVERRIDE) == 0) {
					context.errors.push(new CompileError(token, "property '" + memberClassDef.className() + "#" + member.name() + "' should have 'override' attribute set, declared as 'abstract' in class '" + this.className() + "'"));
					return false;
				}
