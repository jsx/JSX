...
var ClassDefinition = exports.ClassDefinition = Class.extend({
	...
	resolveTypes: function (context) {
		...
		if (this._extendName != null) {
			...
			var baseClass = ClassDefinition.getClass(context.classDefs, baseClassName);
			if (baseClass == null)
				context.errors.push(new CompileError(this._extendName[0], "class '" + baseClassName + "' is not defined"));
		...
		for (var i = 0; i < this._implementNames.length; ++i) {
			...
			var baseClass = ClassDefinition.getClass(context.classDefs, baseClassName);
			if (baseClass == null)
				context.errors.push(new CompileError(this._implementNames[i][0], "interface '" + baseClassName + "' is not defined"));
...
var ArgumentDeclaration = exports.ArgumentDeclaration = LocalVariable.extend({
	...
	instantiate: function (instantiationContext) {
		return new ArgumentDeclaration(this._name, this._type.instantiate(instantiationContext));
	}
