...
var ClassDefinition = exports.ClassDefinition = Class.extend({
	...
	resolveTypes: function (context) {
		...
		if (this._extendName != null) {
			var baseClass = context.parser.lookup(context.errors, this._extendName, this._extendName.getValue());
			if (baseClass == null)
		...
		for (var i = 0; i < this._implementNames.length; ++i) {
			var baseClass = context.parser.lookup(context.errors, this._implementNames[i], this._implementNames[i].getValue());
			var success = true;
			if (baseClass == null) {
