...
var ClassDefinition = exports.ClassDefinition = Class.extend({
	...
	analyze: function (context) {
		...
		// check that the "abstract" variables of mixins are defined by the class
		for (var i = 0; i < this._implementClassDefs.length; ++i) {
			var interfaceDef = this._implementClassDefs[i];
			if ((interfaceDef.flags() & ClassDefinition.IS_MIXIN) != 0)
				for (var j = 0; j < interfaceDef._members.length; ++j)
					if (interfaceDef._members[j] instanceof MemberVariableDefinition
						&& (interfaceDef._members[j].flags() & ClassDefinition.IS_ABSTRACT) != 0)
						if (! this._isAbstractVariableDefined(interfaceDef._members[j], interfaceDef))
							context.errors.push(new CompileError(this.getToken(), "no member definition for 'abstract' variable '" + interfaceDef.className() + "#" + interfaceDef._members[j].name() + "'"));
		}
	...
	_isAbstractVariableDefined: function (member, memberClassDef) {
		for (var i = 0; i < this._members.length; ++i) {
			if (this._members[i].name() == member.name()
				&& this._members[i] instanceof MemberVariableDefinition
				&& (this._members[i] & (ClassDefinition.IS_STATIC & ClassDefinition.IS_ABSTRACT)) == 0) {
				return true;
			}
		}
		if (this._extendClassDef != null && this._extendClassDef._isAbstractVariableDefined(member, memberClassDef))
			return true;
		for (var i = 0; i < this._implementClassDefs.length; ++i) {
			if (this._implementClassDefs[i] == memberClassDef)
				continue;
			if (this._implementClassDefs[i]._isAbstractVariableDefined(member, memberClassDef))
				return true;
		}
		return false;
	},
