...
var ClassDefinition = exports.ClassDefinition = Class.extend({
	...
	resolveTypes: function (context) {
		...
		if (this._extendName != null) {
			var baseClass = context.parser.lookup(this._extendName.getValue());
			if (baseClass.length == 0)
				context.errors.push(new CompileError(this._extendName, "could not find definition"));
			else if (baseClass.length > 1)
				context.errors.push(new CompileError(this._extendName, "multiple candidates"));
			else if ((baseClass[0].flags() & ClassDefinition.IS_FINAL) != 0)
				context.errors.push(new CompileError(this._extendName, "cannot extend a final class"));
			else if ((baseClass[0].flags() & ClassDefinition.IS_INTERFACE) != 0)
				context.errors.push(new CompileError(this._extendName, "cannot extend an interface, use the 'implements' keyword"));
			else if ((baseClass[0].flags() & ClassDefinition.IS_MIXIN) != 0)
				context.errors.push(new CompileError(this._extendName, "cannot extend an mixin, use the 'implements' keyword"));
			else
				this._extendClassDef = baseClass[0];
		} else if (this._className != "Object") {
			var baseClass = context.parser.lookup("Object");
			if (baseClass.length != 1)
				throw new Error("found " + baseClass.length + " definition(s) for Object class");
			this._extendClassDef = baseClass[0];
		}
		// resolve implements
		for (var i = 0; i < this._implementNames.length; ++i) {
			var baseClass = context.parser.lookup(this._implementNames[i].getValue());
			var success = true;
			if (baseClass.length == 0) {
				context.errors.push(new CompileError(this._implementNames[i], "interface '" + this._implementNames[i].getValue() + "' is not defined"));
				success = false;
			} else if (baseClass.length > 1) {
				context.errors.push(new CompileError(this._implementNames[i], "multiple candidates for '" + this._implementNames[i].getValue() + "'"));
				success = false;
			} else if ((baseClass[0].flags() & (ClassDefinition.IS_INTERFACE | ClassDefinition.IS_MIXIN)) == 0) {
				...
				for (var j = 0; j < this._implementClassDefs.length; ++j) {
					if (this._implementClassDefs[j] == baseClass[0]) {
			...
			if (success)
				this._implementClassDefs.push(baseClass[0]);
		}
		// resolve types used
