...
var ClassDefinition = exports.ClassDefinition = Class.extend({
	...
	analyze: function (context) {
		...
		for (var i = 0; i < this._members.length; ++i) {
			...
			if ((member.flags() & ClassDefinition.IS_STATIC) == 0) {
				if (member instanceof MemberVariableDefinition) {
					if (this._extendClassDef != null && ! this._extendClassDef._assertMemberVariableIsDefinable(context, member)) {
						// error found and reported
					} else {
						for (var j = 0; j < this._implementClassDefs.length; ++j)
							if (! this._implementClassDefs[j]._assertMemberVariableIsDefinable(context, member))
								break;
					}
				} else { // MemberFunctionDefinition
					if (this._extendClassDef != null && ! this._extendClassDef._assertMemberFunctionIsDefinable(context, member)) {
						// error found and reported
					} else {
						for (var j = 0; j < this._implementClassDefs.length; ++j)
							if (! this._implementClassDefs[j]._assertMemberFunctionIsDefinable(context, member))
								break;
					}
				}
			}
	...
	_assertMemberVariableIsDefinable: function (context, member) {
		for (var i = 0; i < this._members.length; ++i) {
			if (this._members[i].name() == member.name()) {
				context.errors.push(new CompileError(member.getToken(), "cannot define the property, the name is already used in base class '" + this.className() + "'"));
				return false;
			}
		}
		if (this._extendClassDef != null && ! this._extendClassDef._assertMemberVariableIsDefinable(context, member))
			return false;
		for (var i = 0; i < this._implementClassDefs.length; ++i)
			if (! this._implementClassDefs[i]._assertMemberVariableIsDefinable(context, member))
				return false;
		return true;
	},

	_assertMemberFunctionIsDefinable: function (context, member) {
		for (var i = 0; i < this._members.length; ++i) {
			if (this._members[i].name() != member.name())
				continue;
		...
		if (this._extendClassDef != null && ! this._extendClassDef._assertMemberFunctionIsDefinable(context, member))
			return false;
		for (var i = 0; i < this._implementClassDefs.length; ++i)
			if (! this._implementClassDefs[i]._assertMemberFunctionIsDefinable(context, member))
				return false;
		return true;
	}
