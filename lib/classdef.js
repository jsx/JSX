...
var ClassDefinition = exports.ClassDefinition = Class.extend({
	...
	resolveTypes: function (context) {
		...
		if (this._extendName != null) {
			...
			if (baseClass == null)
				context.errors.push(new CompileError(this._extendName[0], "class '" + this._extendName.getValue() + "' is not defined"));
		...
		for (var i = 0; i < this._implementNames.length; ++i) {
			...
			if (baseClass == null)
				context.errors.push(new CompileError(this._implementNames[i][0], "interface '" + this._implementNames[i].getValue() + "' is not defined"));
