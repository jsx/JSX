...
var _NewExpressionEmitter = exports._NewExpressionEmitter = _OperatorExpressionEmitter.extend({
	...
	emit: function (outerOpPrecedence) {
		...
		this._emitter._emit(classDef.className(), this._expr.getOperatorToken());
		var shouldMangle = false;
		if ((classDef.flags() & ClassDefinition.IS_NATIVE) == 0) {
			switch (this._emitter._findFunctions(classDef, "initialize", false).length) {
			case 0:
				break;
			case 1:
		...
		if (shouldMangle) {
			var ctor = this._expr.getConstructor(); // if there is more than one constructor, we should have that one designated
			if (ctor.getArgumentTypes().length != 0)
				this._emitter._emit(this._emitter._mangleFunctionArguments(ctor.getArgumentTypes()), this._expr.getOperatorToken());
		}
		var args = this._expr.getArguments();
		this._emitter._emit("(", null);
		for (var i = 0; i < args.length; ++i) {
