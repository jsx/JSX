...
var _Util = exports._Util = Class.extend({
	...
	$buildAnnotation: function (template, type) {
		...
		if (closureType == null)
			return "";
		// FIXME use Util.format (once updated to support %x without type)
		return template.replace(/%1/g, closureType);
	}
...
var _ConstructorInvocationStatementEmitter = exports._ConstructorInvocationStatementEmitter = _StatementEmitter.extend({
	...
	emit: function () {
		var ctorName = this._emitter._mangleConstructorName(this._statement.getConstructingClassDef(), this._statement.getConstructorType());
...
var _PropertyExpressionEmitter = exports._PropertyExpressionEmitter = _UnaryExpressionEmitter.extend({
	...
	_emit: function () {
		...
		if (exprType instanceof FunctionType && ! exprType.isAssignable()
			&& (this._expr.getHolderType().getClassDef().flags() & ClassDefinition.IS_NATIVE) == 0)
			this._emitter._emit(this._emitter._mangleFunctionArguments(exprType.getArgumentTypes()), null);
	},
...
var _NewExpressionEmitter = exports._NewExpressionEmitter = _OperatorExpressionEmitter.extend({
	...
	emit: function (outerOpPrecedence) {
		...
		this._emitter._emit("new " + this._emitter._mangleConstructorName(classDef, this._expr.getConstructor()) + "(", this._expr.getOperatorToken());
		var args = this._expr.getArguments();
		for (var i = 0; i < args.length; ++i) {
			if (i != 0)
				this._emitter._emit(", ", null);
...
var JavaScriptEmitter = exports.JavaScriptEmitter = Class.extend({
	...
	emitClassDefinition: function (classDef) {
		...
		try {
			...
			if (ctors.length == 0) {
				// emit the default constructor (with no arguments)
				this._emitConstructor(classDef, null, false);
			} else if (ctors.length == 1 && (classDef.flags() & ClassDefinition.IS_FINAL) != 0) {
				this._emitConstructor(classDef, ctors[0], false);
			} else {
				// emit constructor with no arguments, and then emit constructors with mangled suffixes
				for (var i = 0; i < ctors.length; ++i)
					if (ctors[i].getArgumentTypes().length == 0)
						break;
				this._emitConstructor(classDef, i != ctors.length ? ctors[i] : null, false);
				for (var i = 0; i < ctors.length; ++i)
					if (ctors[i].getArgumentTypes().length != 0)
						this._emitConstructor(classDef, ctors[i], true);
			}
	...
	_emitConstructor: function (classDef, funcDef, isSecondary) {
		// emit prologue
		this._emit("/**\n", null);
		...
		this._emit(" */\n", null);
		this._emit("function " + classDef.className() + (isSecondary ? this._mangleFunctionArguments(funcDef.getArgumentTypes()) : "") + "(", null);
		if (funcDef != null)
		...
		if (funcDef != null)
			this._emitConstructorCalls(funcDef);
		...
		this._emit("};\n\n", null);
		if (isSecondary) {
			this._emit(classDef.className() + this._mangleFunctionArguments(funcDef.getArgumentTypes()) + ".prototype = new " + classDef.className() + ";\n\n", null);
		} else if (classDef.extendClassDef() != null || classDef.implementClassDefs().length != 0) {
			this._emit(classDef.className() + ".prototype = new " + (classDef.extendClassDef() != null ? classDef.extendClassDef().className() : "Object") + ";\n", null);
	...
	_emitConstructorCalls: function (funcDef) {
		try {
	...
	_mangleConstructorName: function (classDef, ctorType) {
		var name = classDef.className();
		var shouldMangle = false;
		if ((classDef.flags() & ClassDefinition.IS_NATIVE) == 0) {
			switch (this._findFunctions(classDef, "initialize", false).length) {
			case 0:
				break;
			case 1:
				if ((classDef.flags() & ClassDefinition.IS_FINAL) == 0)
					shouldMangle = true;
				break;
			default:
				shouldMangle = true;
		...
		if (shouldMangle && ctorType.getArgumentTypes().length != 0) {
			name += this._mangleFunctionArguments(ctorType.getArgumentTypes());
		}
		return name;
	},
	...
	_mangleTypeName: function (type) {
		if (type.equals(Type.voidType))
			...
			if (classDef instanceof InstantiatedClassDefinition) {
				...
				var s = "T" + classDef.getTemplateClassName() + typeArgs.length;
