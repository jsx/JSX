	...
	_emit: function () {
		...
		case "+":
			...
			if (firstExprType.equals(secondExprType) && firstExprType.equals(Type.stringType.toMayBeUndefinedType()))
				this._emitter._emit("\"\" + ", null);
			break;
		// equality operators of JSX are strict equality ops in JS
		case "==": op = "==="; break;
		case "!=": op = "!=="; break;
		}
...
var _NewExpressionEmitter = exports._NewExpressionEmitter = _OperatorExpressionEmitter.extend({
	...
	emit: function (outerOpPrecedence) {
		...
		if (this._emitter._findFunctions(classDef, "initialize", false).length > 1) {
			var ctor = this._expr.getConstructor(); // if there is more than one constructor, we should have that one designated
			if ((classDef.flags() & ClassDefinition.IS_NATIVE) == 0 && ctor.getArgumentTypes().length != 0)
