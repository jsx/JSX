...
var Parser = exports.Parser = Class.extend({
	...
	_classDefinition: function () {
		...
		// done
		if (typeArgs != null)
			this._templateClassDefs.push(new TemplateClassDefinition(className.getValue(), flags, typeArgs, extendName, implementNames, members, this._objectTypesUsed));
		else
			this._classDefs.push(new ClassDefinition(className.getValue(), flags, extendName, implementNames, members, this._objectTypesUsed));
		return true;
	},
	...
	_typeDeclaration: function (allowVoid) {
		...
		if (this._expectOpt("MayBeUndefined") != null) {
			...
			if (this._expect(">") == null)
				return null;
			typeDecl = new MayBeUndefinedType(baseType);
		} else {
		...
		while (this._expectOpt("[") != null) {
			...
			typeDecl = new ParsedObjectType(Type.templateTypeToString("Array", [ typeDecl ]));
	...
	_templateTypeDeclaration: function (className, identifierToken) {
		...
		// return object type
		var objectType = new ParsedObjectType(Type.templateTypeToString(className, types));
		this._objectTypesUsed.push(objectType);
		return objectType;
	},
	...
	_postfixExpr: function () {
		...
		case "as":
			...
			if (type == null)
				return null;
			return new AsExpression(op, expr, type);
		default:
