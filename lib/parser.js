...
var Parser = exports.Parser = Class.extend({
	...
	_expectKeyword: function (expected, messageOpt) {
		...
		this._newError("expected " + expected + " but got " + token + (messageOpt ? messageOpt : ""));
		return null;
	},
	...
	_expectIdentifier: function () {
		...
		if (token != null)
			return token;
		token = this._nextToken(); // revert the unget
		this._newError("expected identifier but got " + token);
		return null;
	},
	...
	_primaryExpr: function () {
		...
		default:
			...
			if (token instanceof StringToken)
				...
				this._newError("expected primary expression, but got " + token);
				return null;
			}
		}
	},
