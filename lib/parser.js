...
var Import = Class.extend({

	initialize: function (x) {
		if (x instanceof Token) {
			this._filenameToken = x;
			this._sourceParser = null;
		} else if (x instanceof Parser) {
			// for built-in classes
			this._filenameToken = null;
			this._sourceParser = x;
		} else {
	...
	setSourceParser: function (parser) {
...
var Parser = exports.Parser = Class.extend({
	...
	_importStatement: function (importToken) {
		...
		if (this._expect(";") == null)
			return false;
		this._imports.push(new Import(filenameToken));
		return true;
	},
	...
	_classDefinition: function () {
		...
		if (this._filename.match(/^lib\/built-in\//) == null && className.getValue().match(/^(Array|Boolean|Date|Hash|Number|Object|RegExp|String)$/) != null) {
			this._errors.push(new CompileError(className, "cannot override declaration of a built-in class"));
			success = false;
		} else {
			if (typeArgs != null) {
				for (var i = 0; i < this._templateClassDefs.length; ++i) {
