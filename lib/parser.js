...
var Parser = exports.Parser = Class.extend({
	...
	_importStatement: function (importToken) {
		...
		for (var i = 0; i < this._imports.length; ++i) {
			if (alias == null) {
				...
				if (this._imports[i].getAlias() == alias.getValue()) {
					this._errors.push(new CompileError(alias, "alias with the same name is already declared"));
					return false;
				}
			}
		}
		// push
		this._imports.push(new Import(filenameToken, alias));
		return true;
	},
	...
	_classDefinition: function () {
		...
		if (this._filename.match(/^lib\/built-in\//) == null && Parser._isBuiltinClassName(className.getValue())) {
			...
			for (var i = 0; i < this._imports.length; ++i) {
				var alias = this._imports[i].getAlias();
				if (alias != null && alias == className.getValue()) {
					this._errors.push(new CompileError(className, "the name has already been used as an import alias"));
					success = false;
				}
			}
			if (typeArgs != null) {
