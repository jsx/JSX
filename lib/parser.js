...
var Parser = exports.Parser = Class.extend({
	...
	_expectKeywordOpt: function (expected) {
		...
		for (var i = 0; i < expected.length; ++i) {
			if (this._input.substring(this._pos, this._pos + expected[i].length) == expected[i]) {
				...
				if (expected[i] == "}")
					debugger;
				return new KeywordToken(expected[i], this._filename, this._pos);
			}
		}
		return null;
	},

	_expectKeyword: function (expected) {
		if (! (expected instanceof Array))
			expected = [ expected ];
	...
	_expectIdentifierOpt: function () {
		...
		return new IdentifierToken(matched[0], this._filename, this._pos);
	},
	...
	_expectStringLiteralOpt: function () {
		...
		if (matched == null)
			return null;
		this._tokenLength = matched[0].length;
		return new StringToken(matched[0], this._filename, this._pos);
	},
	...
	_expectNumberLiteralOpt: function () {
		...
		if (matched == null)
			return null;
		this._tokenLength = matched[0].length;
		return new NumberToken(matched[0], this._filename, this._pos);
	},
	...
	_expectRegExpLiteralOpt: function () {
		...
		if (matched == null)
			return null;
		this._tokenLength = matched[0].length;
		return new RegExpToken(matched[0], this._filename, this._pos);
	},
	...
	_unaryExpr: function () {
		...
		var op = this._expectKeywordOpt([ "delete", "++", "--", "+", "-", "~", "!" ]);
		if (op == null)
	...
	_primaryExpr: function () {
		var token;
		if ((token = this._expectKeywordOpt([ "this", "null", "false", "true", "[", "{", "(" ])) != null) {
	...
	_hashLiteral: function (token) {
		...
		if (this._expectKeywordOpt("}") == null) {
			do {
				...
				// separator
				if ((token = this._expectKeyword([ ",", "}" ])) == null)
					return null;
			} while (token.getValue() == ",");
		}
		var type = null;
