...
var Parser = exports.Parser = Class.extend({
	...
	_classDefinition: function () {
		...
		var flags = 0;
		while (true) {
			var token = this._expect([ "class", "interface", "mixin", "final", "native" ]);
			if (token == null)
				return false;
	...
	_memberDefinition: function (classFlags) {
		...
		if (token.getValue() == "function") {
			...
			return this._functionDefinition(flags, classFlags);
		}
		// member variable decl.
		if ((classFlags & ClassDefinition.IS_INTERFACE) != 0) {
			this._newError("interface cannot have member variables");
			return null;
		}
		...
		if ((classFlags & ClassDefinition.IS_MIXIN) == 0 && (flags & ClassDefinition.IS_ABSTRACT) != 0) {
			this._newError("only mixin variables may have 'abstract' attribute");
			return null;
		}
	...
	_functionDefinition: function (flags, classFlags) {
	...
	_lhsExpr: function () {
		var expr;
		var token = this._expectOpt([ "new", "super" ]);
		if (token != null) {
			if (token.getValue() == "super")
