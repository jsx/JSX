...
var Parser = exports.Parser = Class.extend({
	...
	_classDefinition: function () {
		...
		var flags = 0;
		while (true) {
			var token = this._expect([ "class", "interface", "final", "native" ]);
			if (token == null)
				return false;
		...
		// extends
		var extendName = null;
		if ((flags & ClassDefinition.IS_INTERFACE) == 0) {
		...
		if (typeArgs != null)
			...
			this._classDefs.push(new ClassDefinition(className, className.getValue(), flags, extendName, implementNames, members, this._objectTypesUsed));
		return true;
	},
