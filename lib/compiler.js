...
var Compiler = exports.Compiler = Class.extend({
	...
	compile: function () {
		...
		BooleanType._classDef = this.findParser("lib/built-in/Boolean.jsx").lookup("Boolean")[0];
		NumberType._classDef = this.findParser("lib/built-in/Number.jsx").lookup("Number")[0];
		StringType._classDef = this.findParser("lib/built-in/String.jsx").lookup("String")[0];
		...
		for (var i = 0; i < this._parsers.length; ++i) {
			...
			for (var j = 0; j < imports.length; ++j) {
				...
				if (filenameToken != null) {
					...
					imports[j].setSourceParser(this._parsers[k]);
				}
			}
		}
	},
	...
	_instantiateTemplate: function (errors, parser, request, resolveImmmediately) {
		...
		var classDefs = parser.lookup(concreteClassName);
		if (classDefs.length != 0)
			return classDefs[0];
		// instantiate
		var templateClasses = parser.lookupTemplate(request.getClassName());
		if (templateClasses.length == 0) {
			...
			return null;
		} else if (templateClasses.length > 1) {
			errors.push(new CompileError(request.getToken(), "multiple candidates exist for template class '" + request.getClassName() + "'"));
			return null;
		}
		var classDef = templateClasses[0].instantiate(errors, request);
		if (classDef == null)
			return null;
		// register
