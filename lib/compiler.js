...
var Compiler = exports.Compiler = Class.extend({
	...
	_instantiateTemplates: function (errors) {
		for (var i = 0; i < this._templateInstantiationRequests.length; ++i)
			this._instantiateTemplate(errors, this._templateInstantiationRequests[i]);
	},

	_instantiateTemplate: function (errors, request) {
	...
	_resolveTypes: function (errors) {
		for (var i = 0; i < this._classDefs.length; ++i)
			this._classDefs[i].resolveTypes(new AnalysisContext(errors, this._classDefs, this._instantiateTemplate.bind(this)));
	},

	_analyze: function (errors) {
		for (var i = 0; i < this._classDefs.length; ++i)
			this._classDefs[i].analyze(new AnalysisContext(errors, this._classDefs, this._instantiateTemplate.bind(this)));
	},
