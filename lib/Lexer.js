...
var Lexer = Class.extend({
	...
	tokenize: function(errors) {
		...
		while (pos < this._input.length) {
			...
			var start = this._input.substring(pos);
			var matched;
			if (matched = start.match(Lexer.rxSpace)) {
				tokens.push(new SpaceToken(pos));
			}
			else if (matched = start.match(Lexer.rxStringLiteral)) {
				tokens.push(new StringToken(matched[0], pos));
			}
			else if (matched = start.match(Lexer.rxNumberLiteral)) {
				tokens.push(new NumberToken(matched[0], pos));
			}
			else if (matched = start.match(Lexer.rxRegExpLiteral)) {
				tokens.push(new RegExpToken(matched[0], pos));
			}
			else if (matched = start.match(Lexer.rxOperator)) {
				tokens.push(new KeywordToken(matched[0], pos));
			}
			else if (matched = start.match(Lexer.rxIdent)) {
				if (Lexer.keyword[matched[0]]) {
					tokens.push(new KeywordToken(matched[0], pos));
				} else if (Lexer.reserved[matched[0]]) {
					errors.push("keyword " + matched[0] + " is not supported under current version of JSX");
				} else {
					tokens.push(new IdentifierToken(matched[0], pos));
				}
			}
			else {
