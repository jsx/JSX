...
var Parser = module.exports = Class.extend({
	...
	_registerLocal: function (name, type) {
		...
		this._variables.push(new LocalVariable(name, type));
	},
	...
	_variableStatement: function () {
		if (! this._variableDeclarations())
			return false;
		return this._expectKeyword(";") != null;
	},
	...
	_ifStatement: function () {
		...
		return new IfStatement(expr, onTrueStatements, onFalseStatements);
	},
	...
	_variableDeclarations: function () {
		...
		do {
			...
			var initialValue = null;
			if (this._expectKeywordOpt("="))
				if ((initialValue = this._assignExpr()) == null)
					return null;
			if (this._registerLocal(identifier, type))
				if (initialValue != null)
					this._statements.push(new AssignmentExpression(new IdentifierExpression(identifier), initialValue));
		} while (this._expectKeywordOpt(",") != null);
	},
	...
	_primaryExpr: function () {
		...
		default:
			...
			if (token instanceof StringToken)
				...
				this._newError("FIXME, EOF or something bad");
				return null;
			}
		}
	}
