...
var OperatorExpression = exports.OperatorExpression = Expression.extend({
	...
	assertIsConvertibleTo: function (context, expr, type) {
		...
		if (! exprType.isConvertibleTo(type)) {
			context.errors.push(new CompileError(this._operatorToken, "cannot apply operator '" + this._operatorToken.keyword + "' to type '" + exprType.toString() + "'"));
			return false;
		}
		return true;
	}
...
var IdentifierExpression = exports.IdentifierExpression = Expression.extend({
	...
	analyze: function (context) {
		if ((this._local = context.funcDef.getLocal(this._identifierToken.identifier)) != null) {
			// ok
		} else {
			var classDef = ClassDefinition.getClass(context.classDefs, this._identifierToken.identifier);
			if (classDef == null) {
				context.errors.push(new CompileError(this._identifierToken, "local variable '" + this._identifierToken.identifier + "' is not declared"));
				return false;
			}
...
var UnaryExpression = exports.UnaryExpression = OperatorExpression.extend({
	...
	analyze: function (context) {
		...
		if (this._expr.getType().equals(Type.voidType)) {
			context.errors.push(new Error(this._operatorToken, "cannot apply operator '" + this._operatorToken.keyword + "' against void"));
			return false;
		}
		return true;
	}
...
var IncrementExpression = exports.IncrementExpression = UnaryExpression.extend({
	...
	analyze: function (context) {
		...
		if (exprType == null) {
			...
			context.errors.push(new CompileError(this._operatorToken, "cannot apply operator '" + this._operatorToken.keyword + "' to a non-number"));
			return false;
		}
		return true;
	},
...
var PropertyExpression = exports.PropertyExpression = UnaryExpression.extend({
	...
	analyze: function (context) {
		...
		if ((this._type = classDef.getMemberTypeByName(context.errors, context.classDefs, this._identifierToken.identifier)) == null) {
			context.errors.push(new CompileError(this._identifierToken, "'" + exprType.toString() + "' does not have property named '" + this._identifierToken.identifier) + "'");
			return false;
		}
		return true;
	},
...
var ConditionalExpression = exports.ConditionalExpression = OperatorExpression.extend({
	...
	analyze: function (context) {
		...
		} else if ((typeIfTrue.equals(Type.integerType) && typeIfFalse.equals(Type.numberType))
				|| (typeIfTrue.equals(Type.numberType) && typeIfFalse.equals(Type.integerType))) {
			// specal case
...
var CallExpression = exports.CallExpression = OperatorExpression.extend({
	...
	analyze: function (context) {
		...
		if (! exprType.isCallableWith(argTypes, false)) {
			context.errors.push(new CompileError(this._operatorToken, "cannot call function, arguments mismatch"));
			return false;
		}
		return true;
	},
...
var NewExpression = exports.NewExpression = OperatorExpression.extend({
	...
	analyze: function (context) {
		if (this._name.length != 1)
			throw new Error("FIXME");
		var classDef = ClassDefinition.getClass(context.classDefs, this._name[0].identifier);
		if (classDef == null) {
			context.errors.push(new CompileError(this._name[0], "no class definition for '" + this._name[0].identifier + "'"));
			return false;
		}
		var argTypes = _Util.analyzeArgs(context, this._args);
		if (argTypes == null)
			return false;
		var ctorType = classDef.getMemberTypeByName(context.errors, context.classDefs, "initialize");
		var isCallable = false;
		if (ctorType != null)
			isCallable = ctorType.isCallableWith(argTypes, false);
		else
			isCallable = argTypes.length == 0;
		if (! isCallable) {
			context.errors.push(new CompileError(this._operatorToken, "cannot create an object of type '" + this._name[0].identifier + "', arguments mismatch"));
			return false;
		}
