...
var UnaryExpression = exports.UnaryExpression = OperatorExpression.extend({
	...
	analyze: function (context) {
		...
		if (this._expr.getType().equals(Type.voidType)) {
			context.errors.push(new Error(this._operatorToken, "cannot apply operator '" + this._operatorToken.getValue() + "' against void"));
			return false;
		}
		return true;
	}
...
var PropertyExpression = exports.PropertyExpression = UnaryExpression.extend({
	...
	analyze: function (context) {
		...
		if ((this._type = classDef.getMemberTypeByName(context.errors, context.classDefs, this._identifierToken.getValue())) == null) {
			context.errors.push(new CompileError(this._identifierToken, "'" + exprType.toString() + "' does not have property named '" + this._identifierToken.getValue()) + "'");
			return false;
		}
		return true;
	},
...
var ArrayExpression = exports.ArrayExpression = BinaryExpression.extend({
	...
	analyze: function (context) {
		...
		if (! (this._expr1.getType() instanceof ArrayType)) {
			context.errors.push(new CompileError(this._operatorToken, "cannot apply operator '[]' against non-array"));
			return false;
		}
		if (! this._expr2.getType().isConvertibleTo(Type.integerType)) {
			context.errors.push(new CompileError(this._operatorToken, "index should be a number"));
			return false;
		}
		return true;
	},
	...
	getType: function () {
		return this._expr1.getType().getElementType();
	}
