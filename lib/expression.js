...
var _Util = exports._Util = Class.extend({
	...
	$analyzeArgs: function (errors, classDefs, funcDef, args) {
		var argTypes = [];
		for (var i = 0; i < args.length; ++i) {
			if (! args[i].analyze(errors, classDefs, funcDef))
				return null;
...
var CallExpression = exports.CallExpression = OperatorExpression.extend({
	...
	analyze: function (errors, classDefs, funcDef) {
		if (! this._expr.analyze(errors, classDefs, funcDef))
			return false;
		var argTypes = _Util.analyzeArgs(errors, classDefs, funcDef, this._args);
		if (argTypes == null)
			return false;
		var exprType = this._expr.getType();
...
var NewExpression = exports.NewExpression = OperatorExpression.extend({
	...
	analyze: function (errors, classDefs, funcDef) {
		if (this._name.length != 1)
			throw new Error("FIXME");
		var classDef = ClassDefinition.getClass(classDefs, this._name[0].identifier);
		if (classDef == null) {
			errors.push(new CompileError(this._name[0], "no class definition for '" + this._name[0].identifier + "'"));
			return false;
		}
		var argTypes = _Util.analyzeArgs(errors, classDefs, funcDef, this._args);
		if (argTypes == null)
			return false;
		var ctorType = classDef.getMemberTypeByName(errors, classDefs, "initialize");
		...
		if (! isCallable) {
			errors.push(new CompileError(this._operatorToken, "cannot create an object of type '" + this._name[0].identifier + "', arguments mismatch"));
			return false;
		}
