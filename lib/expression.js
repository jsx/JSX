...
var IdentifierExpression = exports.IdentifierExpression = Expression.extend({
	...
	analyze: function (context) {
		if ((this._local = context.funcDef.getLocal(this._identifierToken.getValue())) != null) {
			...
			var classDefs = context.parser.lookup(this._identifierToken.getValue());
			if (classDefs.length == 0) {
				...
				context.errors.push(new CompileError(this._identifierToken, "multiple candidates for name '" + this._identifierToken.getValue() + "'"));
			}
			this._classDefType = new ClassDefType(classDefs[0]);
		}
		return true;
	},
...
var RegExpLiteralExpression = exports.RegExpLiteralExpression = Expression.extend({
	...
	analyze: function (context) {
		var classDefs = context.parser.lookup("RegExp");
		if (classDefs.length != 1)
			throw new Error("logic flaw: found " + classDefs.length + " definition for RegExp class");
		this._type = new ObjectType(classDefs[0]);
		return true;
	},
...
var NewExpression = exports.NewExpression = OperatorExpression.extend({
	...
	analyze: function (context) {
		var classDefs = context.parser.lookup(this._name.getValue());
		if (classDefs.length == 0) {
			context.errors.push(new CompileError(this._name, "no class definition for '" + this._name.getValue() + "'"));
			return false;
		} else if (classDefs.length > 1) {
			context.errors.push(new CompileError(this._name, "multiple candidates for '" + this._name.getValue() + "'"));
			return false;
		}
