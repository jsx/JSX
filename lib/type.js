...
var FunctionChoiceType = exports.FunctionChoiceType = FunctionType.extend({
	...
	deduceByArgumentTypes: function (context, argTypes) {
		// try an exact match
		for (var i = 0; i < this._types.length; ++i)
			if (this._types[i]._deduceByArgumentTypes(argTypes, true))
				return this._types[i];
		// try loose match
		var matched = [];
		for (var i = 0; i < this._types.length; ++i)
			if (this._types[i]._deduceByArgumentTypes(argTypes, false))
				matched.push(this._types[i]);
		switch (matched.length) {
		case 0:
			context.errors.push(new CompileError(this._operatorToken, "no function with matching arguments"));
			break;
		case 1:
			return matched[0];
		default:
			context.errors.push(new CompileError(this._operatorToken, "result of function resolution using the arguments is ambiguous"));
			break;
		}
		return null;
	},
...
var ResolvedFunctionType = exports.ResolvedFunctionType = FunctionType.extend({
	...
	deduceByArgumentTypes: function (context, argTypes) {
		if (! this._deduceByArgumentTypes(argTypes, false)) {
			context.errors.push(new CompileError(this._operatorToken, "no function with matching arguments"));
			return false;
		}
		return this;
	...
	_deduceByArgumentTypes: function (argTypes, exact) {
