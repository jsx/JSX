...
var ParsedObjectType = exports.ParsedObjectType = ObjectType.extend({
	...
	resolveType: function (context) {
		if (this._classDef == null) {
			var classDefs = context.parser.lookup(this.toString());
			if (classDefs.length == 0)
				context.errors.push(new CompileError(this._token, "'" + this.toString() + "' is not defined"));
			else if (classDefs.length > 1)
				context.errors.push(new CompileError(this._token, "multiple canditates for class '" + this.toString() + "'"));
			this._classDef = classDefs[0];
		}
	},
