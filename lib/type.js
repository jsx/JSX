...
var ParsedObjectType = exports.ParsedObjectType = ObjectType.extend({
	...
	instantiate: function (instantiationContext) {
		...
		if (actualType != undefined)
			return actualType;
		var objectType = new ParsedObjectType(this._className, this._token);
	...
	resolveType: function (context) {
		if (this._classDef == null
			&& (this._classDef = ClassDefinition.getClass(context.classDefs, this._className)) == null)
...
var ParsedTemplateObjectType = exports.ParsedTemplateObjectType = Type.extend({

	initialize: function (className, typeArguments, token) {
		this._className = className;
	...
	instantiate: function (instantiationContext) {
		var actualType = instantiationContext.typemap[this._className];
		if (actualType == undefined)
			actualType = this._className;
		var typeArgs = [];
		for (var i = 0; i < this._typeArguments.length; ++i) {
			var t = this._typeArguments[i].instantiate(instantiationContext);
			if (t == null)
				return null;
			typeArgs[i] = t;
		}
		var objectType = new ParsedObjectType(Type.templateTypeToString(actualType, typeArgs));
		instantiationContext.objectTypesUsed.push(objectType);
		return objectType;
	},

	toString: function () {
		return Type.templateTypeToString(this._className, this._typeArguments);
	}
